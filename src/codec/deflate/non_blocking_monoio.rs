...
impl DeflateWriteState {
    ...
    pub async fn monoio_send_owned_frame<S: AsyncWriteRent + Unpin>(
        &mut self,
        stream: &mut S,
        ...
        if !frame.header().opcode().is_data() {
            return self
                .write_state
                .monoio_send_owned_frame(stream, frame)
                .await
    ...
    pub async fn monoio_send<S: AsyncWriteRent + Unpin>(
        ...
        if payload.is_empty() {
            ...
            return self.monoio_send_owned_frame(stream, frame).await;
        }
        ...
        for (idx, chunk) in parts.into_iter().enumerate() {
            ...
            match (self.com.as_mut(), code.is_data()) {
                (Some(handler), true) => {
                    ...
                    handler
                        .com
                        .compress(&[chunk.as_slice()], &mut output)
                        .map_err(|code| WsError::CompressFailed(code.to_string()))?;
                    ...
                    .to_vec();
                    let (result, _) = stream.write_all(header).await;
                    let _written_bytes = result?;
                    if let Some(mask) = mask {
                        apply_mask(&mut output, mask)
                    };
                    let (result, _) = stream.write_all(output).await;
                    let _written_bytes = result?;
                    if (self.is_server && handler.config.server_no_context_takeover)
                        || (!self.is_server && handler.config.client_no_context_takeover)
                    {
...
impl DeflateReadState {
    async fn monoio_receive_one<S: AsyncReadRent + Unpin>(
        &mut self,
        stream: &mut S,
    ) -> Result<(SimplifiedHeader, Vec<u8>), WsError> {
        let (mut header, data) = self.read_state.monoio_receive(stream).await?;
    ...
    pub async fn monoio_receive<S: AsyncReadRent + Unpin>(
        ...
        loop {
            let (mut header, mut data) = self.monoio_receive_one(stream).await?;
...
pub struct MonoioDeflateCodec<S: AsyncReadRent + AsyncWriteRent> {
    read_state: DeflateReadState,
    write_state: DeflateWriteState,
    stream: S,
}

impl<S: AsyncReadRent + AsyncWriteRent + Unpin> MonoioDeflateCodec<S> {
    /// construct method
    pub fn new(
        stream: S,
    ...
    pub fn factory(req: http::Request<()>, stream: S) -> Result<Self, WsError> {
        ...
        let codec = MonoioDeflateCodec::new(stream, frame_conf, pmd_config, true);
        Ok(codec)
    }
    ...
    pub fn check_fn(key: String, resp: http::Response<()>, stream: S) -> Result<Self, WsError> {
        ...
        let codec = MonoioDeflateCodec::new(stream, Default::default(), pmd_conf, false);
        Ok(codec)
    }
    ...
    pub async fn send(&mut self, code: OpCode, payload: &[u8]) -> Result<(), WsError> {
        self.write_state
            .monoio_send(&mut self.stream, code, payload)
            .await
    }
    ...
    pub async fn text(&mut self, text: &str) -> Result<(), WsError> {
        self.write_state
            .monoio_send(&mut self.stream, OpCode::Text, text.as_bytes())
            .await
    }
...
pub struct MonoioDeflateRecv<S: AsyncReadRent> {
    stream: S,
    read_state: DeflateReadState,
}

impl<S: AsyncReadRent + Unpin> MonoioDeflateRecv<S> {
    /// construct method
...
pub struct MonoioDeflateSend<S: AsyncWriteRent> {
    stream: S,
    write_state: DeflateWriteState,
}

impl<S: AsyncWriteRent + Unpin> MonoioDeflateSend<S> {
    /// construct method
...
impl<R, W, S> MonoioDeflateCodec<S>
where
    ...
    S: AsyncReadRent + AsyncWriteRent + Unpin + Split<R = R, W = W>,
{
    ...
    pub fn split(self) -> (MonoioDeflateRecv<R>, MonoioDeflateSend<W>) {
        let MonoioDeflateCodec {
            stream,
            read_state,
            write_state,
        } = self;
        ...
        (
            MonoioDeflateRecv::new(read, read_state),
            MonoioDeflateSend::new(write, write_state),
        )
    }
}
